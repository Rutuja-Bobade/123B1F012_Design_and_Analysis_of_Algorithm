...
// Scenario: University Timetable Scheduling : A university is facing challenges in scheduling exam timetables due to overlapping student enrollments in multiple courses. To prevent clashes, the university needs to assign exam slots efficiently, ensuring that no two exams taken by the same student are scheduled at the same time.To solve this, the university decides to model the problem as a Graph Coloring Problem, where: 
// 1. Each course is represented as a vertex. 
// 2. An edge exists between two vertices if a student is enrolled in both courses. 
// 3. Each vertex (course) must be assigned a color (time slot) such that no two adjacent vertices share the same color (no two exams with common students are scheduled in the same slot). 
